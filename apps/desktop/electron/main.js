...
function createWindow() {
    ...
    mainWindow.webContents.session.webRequest.onHeadersReceived({ urls: [`http://127.0.0.1:*/*`] }, (details, callback) => {
        callback({
            responseHeaders: {
...
app.whenReady().then(async () => {
    try {
        ...
        dialog.showErrorBox('Error de inicio', `No se pudo iniciar el motor de análisis.\n\nRevisa el log en:\n${log.transports.file.getFile().path}\n\nIntenta reiniciar la aplicación.`);
        app.quit();
        return;
    }
    ...
    ipcMain.handle('analysis:status', async (_, sessionId) => {
        try {
            const res = await fetch(`http://127.0.0.1:${backendPort}/analysis/status/${sessionId}`);
            if (!res.ok) {
                log.warn(`[analysis:status] HTTP ${res.status} para sesión ${sessionId}`);
                return {
                    stage: 'error',
                    percentage: 0,
